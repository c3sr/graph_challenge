...
#include "pangolin/algorithm/tc_vertex_blocks_dyn_low_tri.cuh"
...
template <typename Index> int run(RunOptions &opts) {
  ...
  for (int i = 0; i < opts.iters; ++i) {
    ...
    auto csr = pangolin::CSR<uint64_t>::from_edges(edges.begin(), edges.end(), lowerTriangularFilter);
    ...
    for (int dev : gpus) {
      ...
      counters.push_back(std::move(pangolin::BissonFaticaTC(dev)));
    }
    ...
    LOG(info, "kernel time {}ms", counters[0].kernel_time());
