...
int main(int argc, char **argv) {
...

  {
    ...
    const int edgesPerGPU = (csr.nnz() + gpus.size() - 1) / gpus.size();
  ...
  while(true)
  {
      ...
      { 
        ...
        if(counter.hnumaffected[0]>0)
        {
          ...
          core_binary_indirect<dimBlock><<<dimGridEdges,dimBlock,0,counter.stream()>>>(counter.gDstKP, counter.gnumdeleted, 
            counter.gnumaffected, kmin + i, 0, counter.selectedOut[0],
    ...
    { 
      auto& counter = trussCounters[i];
      counter.setDevice();
      if(counter.selectedOut[0] - counter.hnumdeleted[0] > 0)
      {
        kmax = kmin + i;
        fallBackGPU = i;
      }
      ...
      /*elapsed = (std::chrono::system_clock::now() - start).count() / 1e9;
      nvtxRangePop();
      LOG(info, "Rest of computaion: transfer best k + soft compact per k {}s", elapsed);*/
    } 
