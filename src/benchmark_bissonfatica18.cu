...
template <typename Index> int run(RunOptions &opts) {
  ...
  for (int i = 0; i < opts.iters; ++i) {
    ...
    auto csr = pangolin::CSR<Index>::from_edges(edges.begin(), edges.end(), lowerTriangularFilter);
    ...
    if (opts.readMostly) {
      csr.read_mostly();
      CUDA_RUNTIME(cudaSetDevice(opts.gpu));
    ...
    std::vector<pangolin::BissonFaticaTC> counters;
    LOG(debug, "create device {} counter", opts.gpu);
    pangolin::BissonFaticaTC counter(opts.gpu);
...
int main(int argc, char **argv) {
  ...
  cli = cli | clara::Opt(opts.gpu, "device id")["-g"]("gpu to use");
  ...
  if (wide) {
    return run<int64_t>(opts);
  } else {
    return run<int32_t>(opts);
  }
}
