...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < iters; ++i) {
    ...
    auto csr = upperTriangular ? pangolin::CSR<uint64_t>::from_edges(edges.begin(), edges.end(), upperTriangularFilter)
                               : pangolin::CSR<uint64_t>::from_edges(edges.begin(), edges.end(), lowerTriangularFilter);
    ...
    std::vector<pangolin::VertexBlocksBinaryTC> counters;
    for (int dev : gpus) {
